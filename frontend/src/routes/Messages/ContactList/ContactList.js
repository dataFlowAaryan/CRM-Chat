        ...
        <div className={ styles.wrapper }>
            { Object.values(messages).map((message) => {
                return <div className={ styles.contact } onClick={ () => {
                    ...
                    <div className={ styles.contact_title }>{ message.user.firstName } { message.user.lastName }</div>
                    <div className={ styles.contact_chat }>{ (message.messages.slice(-1)[ 0 ] || { message: "" }).message }</div>
                </div>;
            }) }
